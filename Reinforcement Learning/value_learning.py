...
if __name__ == '__main__':
    ...
    for i in range(1, 10000, 10):
        print(i)
        start = time.time()
        optimal_v, values = value_iteration(env, gamma, max_iterations = i);
        end = time.time()
    ...
    fig_1 = plt.figure()
    ax_1 = plt.axes()
    plt.title('Value Iteration for FrozenLake 4x4')
    ...
    scores = []
    for value in values:
        policy = extract_policy(value, gamma)
    ...
    fig_1 = plt.figure()
    ax_1 = plt.axes()
    plt.title('Value Iteration for FrozenLake 16x32')
    plt.xlabel('Number of Iterations')
    plt.ylabel('Time Spent (ms)')
    plt.plot(range(1, len(time_spent) + 1), time_spent)
