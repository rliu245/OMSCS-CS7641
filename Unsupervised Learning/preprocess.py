...
def PreProcessAdmission(X, y):
    X = X.drop('Serial No.', axis = 1)
    
    gre_sc = StandardScaler()
    X['GRE Score'] = gre_sc.fit_transform(np.array(X['GRE Score']).reshape(-1, 1))
    
    toefl_sc = StandardScaler()
    X['TOEFL Score'] = toefl_sc.fit_transform(np.array(X['TOEFL Score']).reshape(-1, 1))
    
    uniRating_sc = StandardScaler()
    X['University Rating'] = uniRating_sc.fit_transform(np.array(X['University Rating']).reshape(-1, 1))
    
    sop_sc = StandardScaler()
    X['SOP'] = sop_sc.fit_transform(np.array(X['SOP']).reshape(-1, 1))
    
    lor_sc = StandardScaler()
    X['LOR'] = lor_sc.fit_transform(np.array(X['LOR']).reshape(-1, 1))
    
    cgpa_sc = StandardScaler()
    X['CGPA'] = cgpa_sc.fit_transform(np.array(X['CGPA']).reshape(-1, 1))
    
    X['Research'] = np.array(X['Research']).reshape(-1, 1)
